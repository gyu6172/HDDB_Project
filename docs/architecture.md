...
> **서브카테고리 SSOT는 DB `subcategories` 테이블**이며, 분류기 `CATEGORY_DEFS`(11개: sky = bird·space·weather·air_pollution / land = disaster·animal·ground_pollution·insect / sea = marine_life·deep_sea·ocean_pollution)로 시드된다. 프론트(`constants/category.ts`)도 동일한 11개를 노출해 정합한다.
